...
def generate_all_infantry_weapons(
    data_files_directory: str,
) -> Iterator[InfantryWeapon]:
    print("Generating infantry weapon objects")

    # Load and filter infantry weapons
    all_infantry_weapons_data: List[dict] = list(
    ...
    yield from (
        parse_infantry_weapon_data(data=data, fire_groups_data=all_fire_groups_data,)
        for data in filter(
            lambda x: (int(x["item_id"]) not in EXCLUDED_ITEM_IDS),
            all_infantry_weapons_data,
        )
    )


def parse_infantry_weapon_data(
    ...
    try:
        ...
        if item_id in INFANTRY_WEAPONS_DATA_FIXERS:
            INFANTRY_WEAPONS_DATA_FIXERS[item_id](data)
