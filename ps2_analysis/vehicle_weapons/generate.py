...
def generate_vehicle_weapons(data_files_directory: str) -> List[VehicleWeapon]:
    ...
    weapons: List[VehicleWeapon] = parse_vehicle_weapons_data(filtered)
    print(f"Generated {len(weapons)} vehicle weapon objects")
...
def parse_vehicle_weapons_data(data: List[dict]) -> List[VehicleWeapon]:
    ...
    for d in data:
        ...
        try:
                    ...
                    fire_mode: FireMode = FireMode(
                        ...
                        direct_damage_profile=(
                            DamageProfile(
                                ...
                                min_damage_range=optget(
                                    fm, "min_damage_range", float, 0.0
                                ),
                                # Pellets
                                pellets_count=optget(
                                    fm, "fire_pellets_per_shot", int, 1
                                ),
            ...
            vehicle_weapon: VehicleWeapon = VehicleWeapon(
                ...
                # Handling timings
                equip_time=optget(w, "equip_ms", int, 0.0),
                unequip_time=optget(w, "unequip_ms", int, 0.0),
                from_ads_time=optget(w, "from_iron_sights_ms", int, 0.0),
                to_ads_time=optget(w, "to_iron_sights_ms", int, 0.0),
                sprint_recovery_time=optget(w, "sprint_recovery_ms", int, 0.0),
                # Fire groups
